...
import { Image, ImageBackground, Button, Animated } from 'react-native';
...
function Navigator(props) {
  ...
  /****************************/
  const animated_width = new Animated.Value(87);
  const animated_height = new Animated.Value(187);
  ...
  useEffect(() => {
    if(loading){
      Animated.timing(
        animated_width, // The animated value to drive
        {
          opacity: 1,
          toValue: 175, // Animate to opacity: 1 (opaque)
          duration: 0, // Make it take a while
          useNativeDriver: false,
        },
      ).start(); // Starts the animation
      Animated.timing(
        animated_height, // The animated value to drive
        {
          opacity: 1,
          toValue: 375, // Animate to opacity: 1 (opaque)
          duration: 1400, // Make it take a while
          useNativeDriver: false,
        },
      ).start(); // Starts the animation
    }
  ...
  /** RENDER */
  return (
    <ImageBackground style={[cStyles.container,  { opacity: 1}]}  resizeMode={"cover"}>

      {loading  && 
        <View style={{flex:1, alignItems:'center', justifyContent:'center'}}>
          <Animated.Image
            source={Assets.splash2}
            style={{
